...
const MyTicketsTable = forwardRef(({ onTicketAction }, ref) => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      console.log('=== DEBUG: MyTicketsTable.fetchTickets called ===');
      const response = await adminService.getMyTickets();
      ...
      if (response.success) {
        ...
        console.error('Error from getMyTickets:', response.error);
    ...
    if (filters.dateRange !== 'all') {
      ...
      switch (filters.dateRange) {
        case 'last_7_days':
          ...
          filtered = filtered.filter(ticket => new Date(ticket.createdAt) >= sevenDaysAgo);
          break;
        case 'last_30_days':
          const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
          filtered = filtered.filter(ticket => new Date(ticket.createdAt) >= thirtyDaysAgo);
          break;
        case 'this_year':
          const thisYear = now.getFullYear();
          filtered = filtered.filter(ticket => new Date(ticket.createdAt).getFullYear() === thisYear);
          break;
      }
    }
            ...
            <div className="flex">
              <div className="flex-shrink-0">
                <svg className="h-5 w-5 text-red-400" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 101.414 1.414L10 11.414l1.293 1.293a1 101.414-1.414L11.414 10l1.293-1.293a1 100-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                </svg>
              </div>
              <div className="ml-3">
                <h3 className="text-sm font-medium text-red-800">Error loading my tickets</h3>
