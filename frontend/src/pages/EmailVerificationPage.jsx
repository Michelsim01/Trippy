...
const EmailVerificationPage = () => {
    ...
    // Check if user has token but is not authenticated (needs email verification)
    useEffect(() => {
        ...
        // If user is authenticated (email verified), redirect to home
        if (isAuthenticated) {
            navigate('/home')
        }
    ...
    const handleResendVerification = async () => {
        ...
        try {
            const result = await authService.sendVerificationEmail(user.email)
    ...
    const handleCheckVerification = async () => {
        ...
        try {
            const result = await authService.checkEmailVerification(user.email)

            if (result.success) {
                if (result.data.emailVerified) {
                    setMessage('Your email is verified! You can now access all features.')
    ...
    const handleBackToHome = async () => {
        try {
            ...
            if (storedUser) {
                ...
                // Call the cancel endpoint to clean up pending registration
                await authService.cancelPendingRegistration(user.email)
            }
        } catch (error) {
            console.error('Error canceling pending registration:', error)
        ...
        <div className="min-h-screen bg-neutrals-8 flex items-center justify-center px-4">
            <div className="max-w-md w-full">
                ...
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-2xl font-bold text-neutrals-1 mb-2">Verify Your Email</h1>
